...
export default function Sidebar() {
  ...
  const NavContent = () => (
    <>
      ...
      <div className="px-3 pb-4 border-t border-border/40 pt-3">
        ...
        <button
          onClick={() => signOut()}
          className="flex items-center gap-2 w-full px-3 py-2 rounded-xl text-xs text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all duration-200"
        >
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-60 shrink-0 border-r border-border/60 bg-background/50 h-screen sticky top-0">
        <NavContent />
      </aside>
